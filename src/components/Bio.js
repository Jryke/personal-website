import React from "react";

const Bio = () => {
  return (
	  <div id="bio" className="ui center aligned grid container" style={{padding: '3em'}}>
      <h2 className="row">Bio:</h2>
			<div className="row">
				<h3>Professional and personal philosophies:</h3>
			</div>
			<div className="row">
				<h4>work smarter... and harder</h4>
				<p>
					I consider myself to be intelligent, but I also know that intelligence is useless without applying it.  In my professional life (as well as anything that I do), I am a hard worker and I always strive to give my 100% in everything that I do.  I hold to the proverb "any job worth doing is a job worth doing well".  If I am going to put my time and energy into something, I am going to see it through properly; otherwise I am just wasting my time and energy.  Therfore, I take pride in everything I do and I do everything to the best of my ability.
				</p>
			</div>
			<div className="row">
				<h4>Create and be creative:</h4>
				<p>
					Growing up, my father was a carpenter and an architect so we had a woodshop in the basement of our house.  I spent a lot of my childhood in the basement making things (including my own toys or boxes to store my toys).  Among other expriences, I believe that this largely shaped my desire to be creative and build things.  I get a lot of satisfaction out of planning a project, executing it and seeing the desired result at the end.  This has played a big part for me becoming a web developer.  I really enjoy the creative process from beginning to end of building web sites and web apps.
				</p>
			</div>
			<div className="row">
				<h4>Be a lifelong learner:</h4>
				<p>
					I love to learn and I know that learning is the best use of my time.  I believe that if I am not learning (and enjoying what I am learning), I am wasting my time.  While studying my masters degree in Applied Linguistics to become a teacher, I learned not just how to be a good teacher but also to be a good student.
				</p>
			</div>
			<div className="row">
				<h4>Do what you love and love what you do:</h4>
				<p>
					Time is the most valuable thing that we have and we need to take advantage of it as much as possible to be happy.  To take advantage of our time, it is important to work doing something that you love to do, otherwise you are wasting your time doing things that don't contribute to your happiness.  Many people work to make money but don't have the time to enjoy it, this is counter-intuitive.  Enjoying what you do is the key a "successful" lifestyle, not just making money (although money is nice to have too!).
				</p>
			</div>
      <br />
      <h2 className="row">Personal Interests:</h2>
      <p >Music/DJing, Traveling, Languages, History, Geography, Science (interesting topics/news articles?)<br />
			Favorite tv shows, movies or books (music! - Reggae, Champeta) (Peter Pan - lessons I have learned)
			<br />
			Family life
			<br />
			Funny details about habits or interests / What people like about me
			<br />
			Unique personal history - living in Colombia, and other "interesting" things that I have done
			</p>
	  </div>
  )
}

export default Bio
